package com.cheftory.api.recipeinfo.step.dto;
...
public record RecipeStepsResponse(@JsonProperty("steps") List<RecipeStepResponse> steps) {
  ...
  public record RecipeStepResponse(
    ...
    public record RecipeStepDetailResponse(
        @JsonProperty("text") String text, @JsonProperty("start") Double start) {
    ...
    public static RecipeStepResponse from(RecipeStep step) {
      return new RecipeStepResponse(
          ...
          step.getStepOrder(),
          step.getSubtitle(),
          step.getDetails().stream().map(RecipeStepDetailResponse::from).toList(),
          step.getStart());
    }
  }
}
