...
""")
  ...
  order by h.viewedAt desc, h.id desc
""")
    List<RecipeBookmark> findUncategorizedFirst(UUID userId, RecipeBookmarkStatus status, Pageable pageable);
  ...
  order by h.viewedAt desc, h.id desc
""")
    List<RecipeBookmark> findUncategorizedKeyset(
            UUID userId, RecipeBookmarkStatus status, LocalDateTime lastViewedAt, UUID lastId, Pageable pageable);
    ...
    @Query("SELECT COUNT(r) as count "
            ...
            + "AND r.status = :status")
    RecipeBookmarkUnCategorizedCountProjection countByUserIdAndStatus(
