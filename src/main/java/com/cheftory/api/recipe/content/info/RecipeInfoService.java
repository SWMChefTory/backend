...
public class RecipeInfoService {
    ...
    public CursorPage<RecipeInfo> getPopulars(String cursor, RecipeInfoVideoQuery videoQuery) {
        ...
        List<RecipeInfo> rows = first ? popularFirst(videoQuery, probe) : popularKeyset(videoQuery, cursor, probe);
    ...
    private List<RecipeInfo> popularFirst(RecipeInfoVideoQuery videoQuery, Pageable probe) {
        return switch (videoQuery) {
            ...
            case NORMAL, SHORTS -> recipeInfoRepository.findPopularByVideoTypeFirst(
                    RecipeStatus.SUCCESS, videoQuery.name(), probe);
        };
    }

    private List<RecipeInfo> popularKeyset(RecipeInfoVideoQuery videoQuery, String cursor, Pageable probe) {
        CountIdCursor p = countIdCursorCodec.decode(cursor);

        return switch (videoQuery) {
            case ALL -> recipeInfoRepository.findPopularKeyset(RecipeStatus.SUCCESS, p.lastCount(), p.lastId(), probe);
            case NORMAL, SHORTS -> recipeInfoRepository.findPopularByVideoTypeKeyset(
    ...
    private List<RecipeInfo> cuisineKeyset(String tag, String cursor, Pageable probe) {
        CountIdCursor p = countIdCursorCodec.decode(cursor);
        return recipeInfoRepository.findCuisineKeyset(tag, RecipeStatus.SUCCESS, p.lastCount(), p.lastId(), probe);
    }
}
