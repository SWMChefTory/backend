...
public class RecipeHistoryService {
    ...
    public Boolean exist(UUID userId, UUID recipeId) {
        return recipeHistoryRepository.existsByRecipeIdAndUserIdAndStatus(recipeId, userId, RecipeHistoryStatus.ACTIVE);
    }
    ...
    public RecipeHistory getWithView(UUID userId, UUID recipeId) {
        RecipeHistory recipeHistory = recipeHistoryRepository
                ...
                .orElseThrow(() -> new RecipeHistoryException(RecipeHistoryErrorCode.RECIPE_HISTORY_NOT_FOUND));
    ...
    public void unCategorize(UUID categoryId) {
        List<RecipeHistory> viewStatuses =
                recipeHistoryRepository.findByRecipeCategoryIdAndStatus(categoryId, RecipeHistoryStatus.ACTIVE);
    ...
    public CursorPage<RecipeHistory> getCategorized(UUID userId, UUID categoryId, String cursor) {
        ...
        List<RecipeHistory> rows = first
                ? recipeHistoryRepository.findCategorizedFirst(userId, categoryId, RecipeHistoryStatus.ACTIVE, probe)
    ...
    public CursorPage<RecipeHistory> getUnCategorized(UUID userId, String cursor) {
        ...
        List<RecipeHistory> rows = first
                ? recipeHistoryRepository.findUncategorizedFirst(userId, RecipeHistoryStatus.ACTIVE, probe)
    ...
    public List<RecipeHistoryCategorizedCount> countByCategories(List<UUID> categoryIds) {
        List<RecipeHistoryCategorizedCountProjection> projections =
                recipeHistoryRepository.countByCategoryIdsAndStatus(categoryIds, RecipeHistoryStatus.ACTIVE);
        return projections.stream()
                .map(projection -> RecipeHistoryCategorizedCount.of(
