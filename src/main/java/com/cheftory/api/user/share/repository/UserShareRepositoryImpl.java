...
public class UserShareRepositoryImpl implements  UserShareRepository {
	...
	public UserShare shareTx(UUID userShareId, int limit) throws UserShareException {
		UserShare userShare =
				userShareJpaRepository.findById(userShareId).orElseThrow(
						() -> new UserShareException(UserShareErrorCode.USER_SHARE_LIMIT_EXCEEDED)
				);
	...
	public UserShare create(UUID userId, Clock clock) throws UserShareException {
		...
		try {
			...
			return userShareJpaRepository.findByUserIdAndSharedAt(userId, today).orElseThrow(() ->
					new UserShareException(UserShareErrorCode.USER_SHARE_NOT_FOUND));
		}
	}
	 ...
	 */
	@Recover
	public UserShare recover(ObjectOptimisticLockingFailureException e, UUID userId) throws UserShareException {
	 ...
	 */
	@Recover
	public void recover(ObjectOptimisticLockingFailureException e, UUID userId, LocalDateTime sharedAt) throws UserShareException {
		throw new UserShareException(UserShareErrorCode.USER_SHARE_CREATE_FAIL);
	}
