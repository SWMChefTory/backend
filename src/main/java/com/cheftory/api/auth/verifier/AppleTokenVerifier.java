...
public class AppleTokenVerifier {
    ...
    public String getSubFromToken(String identityToken) throws VerificationException {
        try {
            ...
            JWKSet jwkSet = appleTokenClient.fetchJwks();
            JWK jwk = jwkSet.getKeyByKeyId(header.getKeyID());

            if (!(jwk instanceof RSAKey rsaKey)) {
                log.error(
                        "[AppleTokenVerifier] 공개키 타입 오류 - kid: {}, alg: {}", header.getKeyID(), header.getAlgorithm());
            ...
            if (Collections.disjoint(claims.getAudience(), validAudiences)) {
                log.error("[AppleTokenVerifier] 잘못된 aud: {}, 허용된 aud: {}", claims.getAudience(), validAudiences);
                throw new VerificationException(VerificationErrorCode.APPLE_INVALID_AUDIENCE);
            }
