package com.cheftory.api.search.autocomplete;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.anyInt;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.doReturn;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.verify;

import com.cheftory.api.search.exception.SearchException;
import java.util.List;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Nested;
import org.junit.jupiter.api.Test;

@DisplayName("AutocompleteService 테스트")
public class AutocompleteServiceTest {

    private AutocompleteRepository autocompleteRepository;
    private AutocompleteService autocompleteService;

    @BeforeEach
    void setUp() {
        autocompleteRepository = mock(AutocompleteRepository.class);
        autocompleteService = new AutocompleteService(autocompleteRepository);
    }

    @Nested
    @DisplayName("자동완성 검색 (autocomplete)")
    class AutocompleteSearch {

        @Nested
        @DisplayName("Given - 유효한 검색어가 주어졌을 때")
        class GivenValidKeyword {
            String keyword;
            List<Autocomplete> autocompletes;

            @BeforeEach
            void setUp() throws SearchException {
                keyword = "김치";
                Autocomplete a1 =
                        Autocomplete.builder().id("1").text("김치찌개").count(100).build();
                Autocomplete a2 =
                        Autocomplete.builder().id("2").text("김치전").count(80).build();
                Autocomplete a3 =
                        Autocomplete.builder().id("3").text("김치볶음밥").count(60).build();
                autocompletes = List.of(a1, a2, a3);

                doReturn(autocompletes)
                        .when(autocompleteRepository)
                        .searchAutocomplete(any(AutocompleteScope.class), any(String.class), anyInt());
            }

            @Nested
            @DisplayName("When - 검색을 요청하면")
            class WhenSearching {
                List<Autocomplete> result;

                @BeforeEach
                void setUp() throws SearchException {
                    result = autocompleteService.autocomplete(AutocompleteScope.RECIPE, keyword);
                }

                @Test
                @DisplayName("Then - 자동완성 목록을 반환한다")
                void thenReturnsList() throws SearchException {
                    assertThat(result).hasSize(3);
                    assertThat(result.get(0).getText()).isEqualTo("김치찌개");
                    assertThat(result.get(1).getText()).isEqualTo("김치전");
                    assertThat(result.get(2).getText()).isEqualTo("김치볶음밥");

                    verify(autocompleteRepository)
                            .searchAutocomplete(eq(AutocompleteScope.RECIPE), eq(keyword), eq(10));
                }
            }
        }

        @Nested
        @DisplayName("Given - 결과가 없는 검색어일 때")
        class GivenNoResults {
            String keyword;

            @BeforeEach
            void setUp() throws SearchException {
                keyword = "없는검색어";
                doReturn(List.of())
                        .when(autocompleteRepository)
                        .searchAutocomplete(any(AutocompleteScope.class), any(String.class), anyInt());
            }

            @Nested
            @DisplayName("When - 검색을 요청하면")
            class WhenSearching {

                @Test
                @DisplayName("Then - 빈 목록을 반환한다")
                void thenReturnsEmpty() throws SearchException {
                    List<Autocomplete> result = autocompleteService.autocomplete(AutocompleteScope.RECIPE, keyword);
                    assertThat(result).isEmpty();
                    verify(autocompleteRepository)
                            .searchAutocomplete(eq(AutocompleteScope.RECIPE), eq(keyword), eq(10));
                }
            }
        }

        @Nested
        @DisplayName("Given - 일부 일치하는 검색어일 때")
        class GivenPartialMatchKeyword {
            String keyword;
            List<Autocomplete> autocompletes;

            @BeforeEach
            void setUp() throws SearchException {
                keyword = "파";
                Autocomplete a1 =
                        Autocomplete.builder().id("1").text("파스타").count(120).build();
                Autocomplete a2 =
                        Autocomplete.builder().id("2").text("파김치").count(50).build();
                autocompletes = List.of(a1, a2);

                doReturn(autocompletes)
                        .when(autocompleteRepository)
                        .searchAutocomplete(any(AutocompleteScope.class), any(String.class), anyInt());
            }

            @Nested
            @DisplayName("When - 검색을 요청하면")
            class WhenSearching {

                @Test
                @DisplayName("Then - 일치하는 목록을 반환한다")
                void thenReturnsMatching() throws SearchException {
                    List<Autocomplete> result = autocompleteService.autocomplete(AutocompleteScope.RECIPE, keyword);
                    assertThat(result).hasSize(2);
                    assertThat(result.get(0).getText()).isEqualTo("파스타");
                    assertThat(result.get(1).getText()).isEqualTo("파김치");
                    verify(autocompleteRepository)
                            .searchAutocomplete(eq(AutocompleteScope.RECIPE), eq(keyword), eq(10));
                }
            }
        }

        @Nested
        @DisplayName("Given - 결과가 많을 때")
        class GivenManyAutocompleteResults {
            String keyword;
            List<Autocomplete> autocompletes;

            @BeforeEach
            void setUp() throws SearchException {
                keyword = "찌개";
                // 5개만 반환하도록 설정 (limit 테스트)
                autocompletes = List.of(
                        Autocomplete.builder().id("1").text("김치찌개").count(100).build(),
                        Autocomplete.builder().id("2").text("된장찌개").count(90).build(),
                        Autocomplete.builder().id("3").text("부대찌개").count(85).build(),
                        Autocomplete.builder().id("4").text("순두부찌개").count(80).build(),
                        Autocomplete.builder().id("5").text("청국장찌개").count(70).build());

                doReturn(autocompletes)
                        .when(autocompleteRepository)
                        .searchAutocomplete(any(AutocompleteScope.class), any(String.class), anyInt());
            }

            @Nested
            @DisplayName("When - 검색을 요청하면")
            class WhenSearching {

                @Test
                @DisplayName("Then - 제한된 개수만큼 반환한다")
                void thenReturnsLimited() throws SearchException {
                    List<Autocomplete> result = autocompleteService.autocomplete(AutocompleteScope.RECIPE, keyword);
                    assertThat(result).hasSize(5);
                    assertThat(result.getFirst().getText()).isEqualTo("김치찌개");
                    verify(autocompleteRepository)
                            .searchAutocomplete(eq(AutocompleteScope.RECIPE), eq(keyword), eq(10));
                }
            }
        }
    }
}
