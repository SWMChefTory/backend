...
class MarketHeaderPropagatorTest {
  ...
  private final ExchangeFilterFunction propagator =
      (request, next) -> {
  ...
  void shouldAddHeader_whenContextPresent() {
    ...
    ClientRequest req =
        ClientRequest.create(HttpMethod.GET, URI.create("https://example.com/api")).build();
      ...
      verify(exchange)
          .exchange(
              org.mockito.ArgumentMatchers.argThat(
                  r -> "KR".equals(r.headers().getFirst(MarketHeaders.COUNTRY_CODE))));
    }
  }
