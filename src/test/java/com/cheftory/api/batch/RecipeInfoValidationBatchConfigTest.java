...
class RecipeInfoValidationBatchConfigTest {
  ...
  @MockitoBean private RecipeCreditPort recipeCreditPort;
  ...
  void shouldProcessAcrossChunkBoundary() throws Exception {
    ...
    try (var ignored = MarketContext.with(new MarketContext.Info(KOREA, "KR"))) {
      long blockedMeta =
          youtubeMetaRepository.findAll().stream()
              ...
              .count();
      long blockedRecipe =
          recipeInfoRepository.findAll().stream()
    ...
    verify(recipeCreditPort, times(51)).refundRecipeCreate(eq(userId), any(UUID.class), eq(creditCost));
  }
