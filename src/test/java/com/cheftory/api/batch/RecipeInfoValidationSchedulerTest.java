...
class RecipeInfoValidationSchedulerTest {
    ...
    void shouldRunYouTubeValidationJobSuccessfully() throws Exception {
        ...
        when(jobOperator.start(eq(youtubeValidationJob), any(JobParameters.class)))
                .thenReturn(jobExecution);

        scheduler.runYouTubeValidation();

        verify(jobOperator, times(2)).start(eq(youtubeValidationJob), any(JobParameters.class));
    }
    ...
    void shouldRunJobWithNewParametersEachTime() throws Exception {
        ...
        verify(jobOperator, times(4)).start(eq(youtubeValidationJob), any(JobParameters.class));
    }
}
