...
public class RecipeHistoryRepositoryTest extends DbContextTest {
        ...
        class GivenValidRecipeHistory {
            ...
            class WhenSavingRecipeHistory {
                ...
                void thenShouldSaveRecipeHistory() {
                    RecipeHistory savedStatus =
                            repository.findById(recipeHistory.getId()).orElseThrow();
    ...
    class CursorQueries {
        ...
        void shouldFindCategorizedFirst() {
            ...
            when(clock.now()).thenReturn(LocalDateTime.of(2024, 1, 2, 10, 0), LocalDateTime.of(2024, 1, 2, 11, 0));
        ...
        void shouldFindCategorizedKeyset() {
            ...
            Pageable pageable = PageRequest.of(0, 2);
            List<RecipeHistory> result = repository.findCategorizedKeyset(
                    userId, categoryId, RecipeHistoryStatus.ACTIVE, h2.getViewedAt(), h2.getId(), pageable);
        ...
        void shouldFindUncategorizedFirst() {
            UUID userId = UUID.randomUUID();
            when(clock.now()).thenReturn(LocalDateTime.of(2024, 1, 3, 10, 0), LocalDateTime.of(2024, 1, 3, 11, 0));
        ...
        void shouldFindUncategorizedKeyset() {
            ...
            Pageable pageable = PageRequest.of(0, 2);
            List<RecipeHistory> result = repository.findUncategorizedKeyset(
                    userId, RecipeHistoryStatus.ACTIVE, h2.getViewedAt(), h2.getId(), pageable);
        ...
        class GivenValidRecipeIdAndUserId {
            ...
            class WhenCheckingExists {
                ...
                void thenShouldExist() {
                    boolean exists =
                            repository.existsByRecipeIdAndUserIdAndStatus(recipeId, userId, RecipeHistoryStatus.ACTIVE);
        ...
        class GivenValidRecipeIdAndUserId {
            ...
            class WhenFindingByRecipeIdAndUserId {
                ...
                void thenShouldReturnRecipeHistory() {
                    RecipeHistory foundStatus = repository
        ...
        class GivenNonExistentRecipeIdAndUserId {
            ...
            class WhenFindingByRecipeIdAndUserId {
                ...
                void thenShouldReturnEmpty() {
                    assertThat(repository.findByRecipeIdAndUserIdAndStatus(
        ...
        class GivenValidUserId {
            ...
            class WhenFindingRecentRecipeHistoryesByUserId {
                ...
                void thenShouldReturnRecentRecipeHistoryesByUserId() {
                    ...
                    assertThat(statuses.get(0).getViewedAt())
                            .isAfter(statuses.get(1).getViewedAt());
                }
            }
            ...
            class WhenFindinTengRecentRecipeHistoryesByUserId {
                ...
                void thenShouldReturnRecentRecipeHistoryesByUserId() {
                    ...
                    for (int i = 0; i < statuses.size() - 1; i++) {
                        assertThat(statuses.get(i).getViewedAt())
                                .isAfter(statuses.get(i + 1).getViewedAt());
                    }
                }
                ...
                void thenShouldApplyPaginationCorrectly() {
                    ...
                    assertThat(firstPageContent.get(9).getRecipeId()).isEqualTo(recipeIds.get(10)); // 10번째로 최근
                    ...
                    assertThat(secondPageContent.get(0).getRecipeId()).isEqualTo(recipeIds.get(9)); // 11번째로 최근
                ...
                void thenShouldReturnCorrectPageInfoForEmptyResult() {
                    ...
                    Page<RecipeHistory> emptyPage =
                            repository.findByUserIdAndStatus(nonExistentUserId, RecipeHistoryStatus.ACTIVE, pageable);
                ...
                class WhenFindingAllRecipeHistories {
                    ...
                    void thenShouldReturnAllHistories() {
                        ...
                        assertThat(result.getContent())
                                .allMatch(h -> h.getUserId().equals(userId));

                        // 카테고리가 있는 것과 없는 것이 모두 포함되어야 함
                        long withCategory = result.getContent().stream()
                                .filter(h -> h.getRecipeCategoryId() != null)
                                .count();
                        long withoutCategory = result.getContent().stream()
                                .filter(h -> h.getRecipeCategoryId() == null)
                                .count();
            ...
            class GivenDeletedHistoriesIncluded {
                ...
                class WhenFindingActiveHistories {
                    ...
                    void thenShouldReturnOnlyActiveHistories() {
                        ...
                        assertThat(result.getContent()).allMatch(h -> h.getStatus() == RecipeHistoryStatus.ACTIVE);
                    }
                }
            }
                ...
                class WhenFindingSpecificUserHistories {
                    ...
                    void thenShouldReturnOnlySpecificUserHistories() {
                        ...
                        assertThat(user1Result.getContent())
                                .allMatch(h -> h.getUserId().equals(userId1));
                        ...
                        assertThat(user2Result.getContent())
                                .allMatch(h -> h.getUserId().equals(userId2));
                    }
                }
            }
        }
        ...
        class CountRecipeHistoryesByCategories {
            ...
            class GivenRecipeHistoryesInMultipleCategories {
                ...
                class WhenCountingRecipeHistoryesByCategories {
                    ...
                    void thenShouldReturnCorrectCountsForEachCategory() {
                        ...
                        RecipeHistoryCategorizedCountProjection count1 = counts.stream()
                                .filter(c -> c.getCategoryId().equals(categoryId1))
                                .findFirst()
                                .orElseThrow();
                        assertThat(count1.getCount()).isEqualTo(3);

                        RecipeHistoryCategorizedCountProjection count2 = counts.stream()
            ...
            class GivenNonExistentRecipeCategoryIds {
                ...
                class WhenCountingRecipeHistoryesByNonExistentCategories {
                    ...
                    @DisplayName("Then - 빈 결과를 반환해야 한다")
                    void thenShouldReturnEmpty() {
                        List<RecipeHistoryCategorizedCountProjection> counts = repository.countByCategoryIdsAndStatus(
            ...
            class GivenValidRecipeIdsAndUserId {
                ...
                class WhenFindingRecipeHistoryesByRecipeIdsAndUserId {
                    ...
                    void thenShouldReturnRecipeHistoryList() {
                        List<RecipeHistory> statuses = repository.findByRecipeIdInAndUserIdAndStatus(
            ...
            class GivenOtherUserViewedRecipes {
                ...
                class WhenFindingSpecificUserRecipeHistoryes {
                    ...
                    void thenShouldReturnOnlySpecificUserStatuses() {
                        List<RecipeHistory> user1Statuses = repository.findByRecipeIdInAndUserIdAndStatus(
                                recipeIds, userId1, RecipeHistoryStatus.ACTIVE);

                        assertThat(user1Statuses).hasSize(3);
                        assertThat(user1Statuses)
                                .allMatch(status -> status.getUserId().equals(userId1));

                        List<RecipeHistory> user2Statuses = repository.findByRecipeIdInAndUserIdAndStatus(
                                recipeIds, userId2, RecipeHistoryStatus.ACTIVE);

                        assertThat(user2Statuses).hasSize(3);
                        assertThat(user2Statuses)
                                .allMatch(status -> status.getUserId().equals(userId2));
                    }
                }
            }
            ...
            class GivenDeletedViewStatuses {
                ...
                class WhenFindingActiveStatuses {
                    ...
                    void thenShouldReturnOnlyActiveStatuses() {
                        ...
                        assertThat(statuses).hasSize(2);
                        assertThat(statuses).allMatch(status -> status.getStatus() == RecipeHistoryStatus.ACTIVE);
                    }
                }
            }
