...
public class RecipeCategoryServiceTest {
      ...
      class WhenCreatingRecipeCategory {
        ...
        void thenShouldCreateRecipeCategoryCorrectly() {
          ...
          assertThat(categoryId)
              .isNotNull()
              .isEqualTo(recipeCategory.getId());
          verify(recipeCategoryRepository).save(argThat(category ->
    ...
    class GivenInvalidParameters {
      ...
      class WhenCreatingRecipeCategory {
        ...
        void thenShouldThrowException() {
          ...
          RecipeCategoryException exception = assertThrows(RecipeCategoryException.class, () -> {
  ...
  class FindUserRecipeCategories {
    ...
    class GivenValidUserId {
      ...
      void setUp() {
        ...
        )).when(recipeCategoryRepository).findAllByUserIdAndStatus(userId, RecipeCategoryStatus.ACTIVE);
      }

      @Nested
      @DisplayName("When - 사용자의 레시피 카테고리를 조회한다면")
      class WhenFindingUserRecipeCategories {
        ...
        void thenShouldReturnUserRecipeCategories() {
          ...
          assertThat(categories)
              .hasSize(2)
              .allMatch(category -> category.getUserId().equals(userId) &&
                  category.getStatus() == RecipeCategoryStatus.ACTIVE);
        }
      }
    }
  ...
  class DeleteRecipeCategory {
    ...
    class GivenExistingCategoryId {
      ...
      void setUp() {
        ...
        doReturn(Optional.of(recipeCategory)).when(recipeCategoryRepository).findById(recipeCategoryId);
      }
