...
public class RecipeControllerTest extends RestDocsTest {
    ...
    class GivenValidUserId {
      ...
      class WhenRequestingRecentRecipes {
        ...
        @Nested
        @DisplayName("When - null 필드가 포함된 레시피 전체 정보를 조회할 때")
        class WhenRequestingRecipeDetailWithNullFields {

          private FullRecipeInfo fullRecipeInfoWithNulls;
          private UUID recipeId;

          @BeforeEach
          void setUp() {
            // null 필드가 포함된 FullRecipeInfo 생성
            fullRecipeInfoWithNulls = FullRecipeInfo.of(
                RecipeStatus.COMPLETED, // 레시피 상태는 유효
                null, // videoInfo가 null
                null, // ingredientsInfo가 null
                null, // recipeStepInfos가 null
                null  // recipeViewStatusInfo가 null
            );

            recipeId = UUID.randomUUID();
            when(recipeService.findFullRecipe(recipeId, userId)).thenReturn(fullRecipeInfoWithNulls);
          }

          @Test
          @DisplayName("Then - null 필드들이 JSON에서 null로 반환된다")
          void thenShouldReturnNullFieldsAsNull() {
            var response = given()
                .contentType(ContentType.JSON)
                .attribute("userId", userId.toString())
                .header("Authorization", "Bearer accessToken")
                ...
                .apply(document(
                    ...
                    responseFields(
                        ...
                        fieldWithPath("video_info").description("레시피 비디오 정보 (null 가능)").optional(),
                        fieldWithPath("ingredients_info").description("레시피 재료 정보 (null 가능)").optional(),
                        fieldWithPath("recipe_steps").description("레시피 단계 정보 (null 가능)").optional(),
                        fieldWithPath("view_status").description("레시피 시청 상태 정보 (null 가능)").optional()
                    )
                ));
            ...
            // RecipeStatus는 null이 아님
            assertThat(responseBody.getString("recipe_status")).isEqualTo(RecipeStatus.COMPLETED.name());

            // null 필드들 검증
            assertThat((Object) responseBody.get("video_info")).isNull();
            assertThat((Object) responseBody.get("ingredients_info")).isNull();
            assertThat((Object) responseBody.get("recipe_steps")).isNull();
            assertThat((Object) responseBody.get("view_status")).isNull();
          }
        }

        @Nested
        @DisplayName("When - 일부 필드만 null인 레시피 정보를 조회할 때")
        class WhenRequestingRecipeDetailWithPartialNulls {

          private FullRecipeInfo partialNullRecipeInfo;
          private UUID recipeId;

          @BeforeEach
          void setUp() {
            // videoInfo만 유효하고 나머지는 null인 경우
            partialNullRecipeInfo = FullRecipeInfo.of(
                RecipeStatus.COMPLETED,
                RecipeControllerTestData.videoInfo, // 유효한 videoInfo
                null, // ingredientsInfo는 null
                List.of(), // 빈 리스트
                null  // recipeViewStatusInfo는 null
            );

            recipeId = UUID.randomUUID();
            when(recipeService.findFullRecipe(recipeId, userId)).thenReturn(partialNullRecipeInfo);
          }

          @Test
          @DisplayName("Then - 유효한 필드는 정상 반환되고 null 필드는 null로 반환된다")
          void thenShouldReturnMixedNullAndValidFields() {
            var response = given()
                .contentType(ContentType.JSON)
                .attribute("userId", userId.toString())
                .header("Authorization", "Bearer accessToken")
            ...
            // videoInfo는 유효해야 함
            assertThat((Object) responseBody.get("video_info")).isNotNull();
            assertThat(responseBody.getString("video_info.video_id")).isNotNull();
            ...
            // 빈 리스트 검증
            assertThat(responseBody.getList("recipe_steps")).isNotNull();
            assertThat(responseBody.getList("recipe_steps")).isEmpty();
          }
        }
